...
兼容格式会自动切换为所需字段组合：增强 LRC 与 ESLRC 省略行 `end`、`agent`、`line`、逐字 `end`、首个节拍标签，并将背景人声改为普通行（或省略），随后在行尾追加结束时间戳；QRC 与 Lys 改用毫秒与后置 `(开始,持续时间)` 标签。根据 v2.3 规范，增强 LRC / ESLRC 的 `.lrc` 输出会强制保留 `[Lyrics Next]`、`[version:2.3]` 和对应格式声明；QRC 与 Lys 则可在无内嵌附属歌词时省略格式声明。
